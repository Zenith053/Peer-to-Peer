...
void handle_download_file(const vector<string>& tokens) {
    string group_id = tokens[1];
    string file_name = tokens[2];
    string destination_path = tokens[3];

    // Request file from server
    string request = "download " + group_id + " " + file_name;
    sendMessage(global_sock, request);

    // 1️⃣ Receive header (file size)
    string header = recvMessage(global_sock);
    if (header.empty()) {
        cerr << "Failed to receive header from server" << endl;
        return;
    }
    ...
    cout << "File size: " << file_size << ", Total pieces: " << total_pieces << endl;
    ...
    if (lseek(fd, (off_t)file_size - 1, SEEK_SET) == -1) {
    ...
    // 3️⃣ Receive piece info from server
    vector<tuple<int,string,string,string,int>> pieces; // piece_no, username, ip, file_path, port
    while (true) {
        string msg = recvMessage(global_sock);
        if (msg.empty()) {
            cerr << "Connection closed while receiving piece info" << endl;
            return;
        }
        istringstream line_stream(msg);
        string line;
        ...
        if (msg.find("END_OF_FILE") != string::npos) break;
    }
    ...
    for (const auto& p : pieces) {
        ...
        if (piece_no < 0 || piece_no >= total_pieces) continue;
        PeerInfo info{get<1>(p), get<2>(p), get<4>(p), get<3>(p)};
        peer_list[piece_no].push_back(info);
    }

    // 5️⃣ Launch worker threads
    int THREADS = 4;
    next_piece = 0; // reset global piece counter
    vector<thread> workers;
    for (int i = 0; i < THREADS; i++) {
        workers.emplace_back(download_piece, file_size, file_name, total_pieces, ref(peer_list));
    }

    for (auto& t : workers) t.join();
    cout << "Download complete: " << destination_path << endl;
}
